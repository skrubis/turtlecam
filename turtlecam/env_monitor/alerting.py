...
class EnvAlertMonitor:
    ...
    def __init__(self, 
                 ...
                 high_temp_safety_duration_sec=300):
        ...
        self.high_temp_safety_duration_sec = high_temp_safety_duration_sec
    ...
    def _monitoring_loop(self):
        ...
        try:
            ...
            logger.error(f"Unhandled error in environmental monitoring loop: {e}", exc_info=True)
        finally:
    ...
    def _check_conditions(self):
        ...
        # Check for high temperature
            ...
            # Check for safety action threshold
            readings_for_safety_trigger = int(
                self.high_temp_safety_duration_sec / self.check_interval
            )
            if self.consecutive_high_temp_readings >= readings_for_safety_trigger:
    ...
    def _trigger_high_temp_safety(self, temperature):
        ...
        try:
            ...
            # Also send an alert
            if self.telegram_sender:
                safety_duration_min = self.high_temp_safety_duration_sec / 60
                message = (
                    "🚨 *CRITICAL TEMPERATURE ALERT* 🚨\n\n"
                    f"Temperature has been above {self.temp_high_threshold}°C for over {safety_duration_min:.0f} minutes!\n"
                    f"Current temperature: *{temperature:.1f}°C*\n\n"
                    "Safety measures have been activated."
                )
                self.telegram_sender.send_message(message, parse_mode=ParseMode.MARKDOWN)
